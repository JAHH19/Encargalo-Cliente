...
public class Database extends SQLiteAssetHelper {
    ...
    public void realizarPedido(String idpedido, Activity activity){
        ...
        for (ItemListOrder order:cart){
            String URL2 = "http://104.215.90.225/APIS/cliente/ingresarlistapedido.php?idtiendaproducto="+order.getIdProducto()+"&idpedido="+idpedido+"&cantidad="+order.getCant()+"&preciouniMX="+order.getPrecio()+"&subtotal="+String.valueOf((Double.parseDouble(order.getCant())*Double.parseDouble(order.getPrecio())*100)/100.0);
